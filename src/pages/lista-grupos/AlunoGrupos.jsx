...
import GrupoVerModal from '../../components/grupo-card/GrupoVerModal';
...
const AlunoGrupos = () => {
  ...
  useEffect(() => {
    const grupo = grupos.filter(grupo => grupo.membros.map(membro => membro._id).includes(user.id))[0];
    setMeuGrupo(grupo);
  }, [grupos])
    ...
    <div className='bg-base-200'>
      ...
      <div className='flex md:relative justify-center pt-[75px]'>
        <div className='flex justify-center items-center md:items-stretch flex-col md:flex-row md:left-[50px] md:w-[92vw]'>
          ...
          <div className="bg-base-100 md:ml-[280px] w-[85vw] h-[85vh] shadow z-[1] rounded-xl md:mb-8 mb-20">
            ...
            <div className="flex flex-col md:flex-row justify-start w-full">
              <label className="input input-bordered mx-6 mt-6 mb-4 md:w-[33vw] border-transparent flex items-center bg-base-300 gap-3">
                <svg
              ...
              <button disabled={meuGrupo === undefined} onClick={()=>document.getElementById(meuGrupo._id).showModal()} className="btn btn-neutral text-neutral-content md:text-lg rounded mt-0 mx-6 mb-6 md:mt-6 md:w-[10vw]">Meu Grupo</button>
            </div>
            ...
            <ul className="list-none overflow-y-auto h-[68vh] bg-base-100 md:pl-4 md:pr-2 pl-4 pr-4 rounded-lg">
              {filteredGroups.length > 0 ? (filteredGroups.map((grupo, index) => {
                return (
                  <AlunoGrupoCard key={index} grupo={grupo} triggerReload={get_groups} />
                )
              })) : (
