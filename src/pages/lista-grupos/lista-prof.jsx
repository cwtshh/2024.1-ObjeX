...
const ListaGruposProf = () => {
  ...
  const get_groups = async() => {
    await axios.get(`${GRUPO_ENDPOINT}/${user.turma._id}`).then((res) => {
      ...
      ToastifyNotificate({ message: 'Erro ao buscar grupos', type: 'error' });
    });
  };

  const get_turmas = async() => {
    await axios.get(`${API_BASE_URL}/turma/${user.turma._id}`, {
      headers: {
        Authorization: `Bearer ${token}`}
    }).then(res => {
      setTurmaProf(res.data);
    }).catch(err => {
      ToastifyNotificate({ message: 'Erro ao buscar turmas', type: 'error' });
    })
  };
  ...
  const create_grupo = async(e) => {
    ...
    await axios.post(`${GRUPO_ENDPOINT}/create`, {
      ...
      ToastifyNotificate({ message: 'Grupo criado com sucesso', type: 'success' });
      setNome("");
      setCapacidade("");
      setDescricao("");
      ...
      ToastifyNotificate({ message: 'Erro ao criar grupo', type: 'error' });
      document.getElementById('my_modal_2').close()
    });
  };
      ...
      <div className='flex md:relative justify-center pt-[75px]'>
        <div className='flex justify-center items-center md:items-stretch flex-col md:flex-row md:left-[50px] md:w-[92vw]'>
          ...
          <div className="bg-base-100 md:ml-[280px] w-[85vw] h-[85vh] shadow z-[1] rounded-xl md:mb-8 mb-20">
            ...
            <ul className='list-none overflow-y-auto h-[68vh] bg-base-100 md:pl-4 md:pr-2 pl-4 pr-4 mx-6 rounded-lg'>
