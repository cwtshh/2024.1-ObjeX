...
const AtividadeCard = ({ atividade }) => {
    ...
    const goToAtividade = () => {
        switch(atividade.type) {
            ...
            case 'image':
                navigate(`/atividade/imagem/${atividade._id}`);
                break;

            case 'text':
                navigate(`/atividade/texto/${atividade._id}`);
                break;
        ...
        <li key={atividade._id} className="p-4 m-2 bg-base-300 rounded-lg">
            <div className="flex md:flex-row flex-col justify-between md:items-center items-middle">
                ...
                <div className='flex flex-row md:w-[10vw] justify-between'>
                    <Link onClick={()=>document.getElementById(atividade._id).showModal()}>
                        <svg
                    ...
                    <div onClick={goToAtividade} className='btn btn-primary text-base-100 rounded-lg'>Submeter</div>
                </div>
            </div>
            <AtividadeVerModal atividade={atividade} />
        </li>
    )
}
